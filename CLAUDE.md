...
- **dozzle** (port 8503) — optional log-viewing sidecar (`amir20/dozzle`, mounts the Docker socket read-only). Not routed through nginx. backend-service and agent-service both log structured JSON to stdout (see `logging_config.py`/`src/core/logging.py`) whenever stdout isn't a TTY, which Dozzle parses; use its SQL Engine (DuckDB-in-browser, `Cmd+Shift+F`) to filter on fields like `session_id`/`request_id` (e.g. `SELECT * FROM logs WHERE session_id = '...'`).
...
`classifier-training/` retrains the `classify.py` category model. `train.py` reads `db_backup/csv_exports/budget_tracker.csv` (regenerate it via `restore_latest_backup.py` first) and writes `classifier.joblib`/`label_encoder.joblib` straight into `backend-service/models/` — the directory the backend Docker image copies in, so no manual copying is needed to ship a retrained model. `test.py` sanity-checks predictions locally using the same load/predict path as `classify.py`. Retraining is manual only (no cron); a running backend-service needs a restart/redeploy to pick up a newly trained model, since `classify.py` caches it in-process.
